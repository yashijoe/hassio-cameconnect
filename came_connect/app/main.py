...
@app.get("/debug/hunt_cycles/{device_id}")
def hunt_cycles(device_id: int, target: int = 90169):
    """
    Scans all States[].Data from /automations/{id}/status to find 'target'
    using u16/u24/u32 (LE/BE) and BCD (1..4 bytes) over sliding windows.
    Returns every hit with State index, CommandId, offset, kind, and bytes.
    """
    access, base = ensure_token()
    url = f"{base}/automations/{device_id}/status"
    r = _request_with_refresh("GET", url)
    ...
    data_root = (r.json() or {}).get("Data") or {}
    states = data_root.get("States") or []
    hits = []

    def bcd_val(arr, i, n, packed=True):
        # packed=True: 2 digits per byte (hi nibble + lo nibble)
        # packed=False: 1 digit per byte (rare, ma proviamo)
        if packed:
            val = 0
            for k in range(n):
                b = arr[i+k]
            ...
            # 1 digit per byte (solo 0..9 ammessi)
            val = 0
            for k in range(n):
                d = arr[i+k]
                if d > 9:
    ...
    def scan_array(arr, meta):
        L = len(arr)
        # u16
        for i in range(L-1):
            u16_le = (arr[i+1] << 8) | arr[i]
            u16_be = (arr[i] << 8) | arr[i+1]
            if u16_le == target: hits.append({**meta, "kind":"u16_le", "i":i, "bytes":arr[i:i+2]})
            if u16_be == target: hits.append({**meta, "kind":"u16_be", "i":i, "bytes":arr[i:i+2]})
        # u24
        for i in range(L-2):
            u24_le = (arr[i+2] << 16) | (arr[i+1] << 8) | arr[i]
            u24_be = (arr[i] << 16) | (arr[i+1] << 8) | arr[i+2]
            if u24_le == target: hits.append({**meta, "kind":"u24_le", "i":i, "bytes":arr[i:i+3]})
            if u24_be == target: hits.append({**meta, "kind":"u24_be", "i":i, "bytes":arr[i:i+3]})
        # u32
        for i in range(L-3):
            u32_le = (arr[i+3] << 24) | (arr[i+2] << 16) | (arr[i+1] << 8) | arr[i]
            u32_be = (arr[i] << 24) | (arr[i+1] << 16) | (arr[i+2] << 8) | arr[i+3]
            if u32_le == target: hits.append({**meta, "kind":"u32_le", "i":i, "bytes":arr[i:i+4]})
            if u32_be == target: hits.append({**meta, "kind":"u32_be", "i":i, "bytes":arr[i:i+4]})
        # BCD packed 1..4 bytes
        for i in range(L):
            for n in (1,2,3,4):
                if i+n <= L:
                    v = bcd_val(arr, i, n, packed=True)
                    if v == target:
                        hits.append({**meta, "kind":f"bcd{n}_packed", "i":i, "bytes":arr[i:i+n]})
        # BCD unpacked 3..6 bytes (90169 ha 5 cifre)
        for i in range(L):
            for n in (3,4,5,6):
                if i+n <= L:
                    v = bcd_val(arr, i, n, packed=False)
                    if v == target:
                        hits.append({**meta, "kind":f"bcd{n}_unpacked", "i":i, "bytes":arr[i:i+n]})

    for idx, st in enumerate(states):
        arr = (st or {}).get("Data")
        if isinstance(arr, list) and arr:
            scan_array(arr, {"state_index": idx, "command_id": st.get("CommandId")})
